...
func testExpectedObject(
	...
	case []int:
		...
		if len(array.Elements) != len(expected) {
			t.Errorf("wrong num of elements. got=%d, want=%d", len(array.Elements), len(array.Elements))
			return
		}
		for i, expectElm := range expected {
			err := testIntegerObject(int64(expectElm), array.Elements[i])
			if err != nil {
				t.Errorf("testIntegerObj failed: %s", err)
			}
		}
...
func TestCallingFunctionsWithoutRetVal(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
			let noReturn = fn() { };
...
func TestFirstClassFn(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
			let returnsOne = fn() { 1; };
			let returnsOneRunner = fn() { returnsOne; };
			returnsOneRunner()();
			`,
			expected: 1,
		},
...
func TestCallingFuncWithBinding(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
...
func TestBuiltinFunc(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			`len("one", "two")`,
			&object.Error{Message: "wrong number of args: got=2, want=1"},
		},
		{`len([1,2,3])`, 3},
		{`len([])`, 0},
